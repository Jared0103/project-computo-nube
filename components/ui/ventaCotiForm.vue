<template>
  <div>
    <button class="add-contact-btn" @click="GoAddVentas">
      <img
        ...
        class="add-icon"
      >
      <span>Agregar nueva cotizacion</span>
    </button>
    ...
    <section v-else class="contacts-table">
      <header class="table-header">
        <span class="header-cell">Numero</span>
        <span class="header-cell">Cliente</span>
        <span class="header-cell">Creacion</span>
        <span class="header-cell">Vencimiento</span>
      ...
      <article v-for="contact in contacts" :key="contact.id" class="table-row">
        <div class="contact-info">
          <span class="cell">{{ contact.numero }}</span>
          <span class="cell">{{ contact.cliente }}</span>
          <span class="cell">{{ contact.creacion }}</span>
          <span class="cell">{{ contact.vencimiento }}</span>
          <span class="cell">{{ contact.estado }}</span>
          <span class="cell">{{ contact.total }}</span>
        </div>
      ...
      <v-card class="dialog-card">
        <v-card-title class="pa-3 ma-3">
          <v-row align="center" justify="center">
            <span class="dialog-title">Borrar Venta</span>
          </v-row>
        </v-card-title>
        <v-card-text class="pa-3 ma-3">
          <v-row align="center" justify="center">
            <span class="dialog-message">¿Estás seguro de que quieres eliminar esta venta?</span>
          </v-row>
        </v-card-text>
...
export default defineComponent({
  ...
  setup () {
    ...
    const fetchContacts = async () => {
      try {
        const response = await axios.get('http://localhost:3020/api/v1/cotizaciones')
        contacts.value = response.data.cotizaciones
      } catch (err) {
    ...
    const handleContactSubmit = async (data) => {
      if (data._id) {
        try {
          await axios.put(`http://localhost:3020/api/v1/cotizaciones/update/${data._id}`, data)
    ...
    const borralo = async () => {
      ...
      try {
        const response = await axios.delete(`http://localhost:3020/api/v1/cotizaciones/delete/${idBorrar.value}`)
